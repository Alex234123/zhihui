  ...
  <div class="equipment-management">
    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px" class="custom-form">
        ...
        <el-form-item label="联系电话" prop="operatorPhone" class="form-item">
          <el-input v-model="form.operatorPhone" placeholder="选择负责人后自动填充" class="form-input" disabled />
        </el-form-item>
...
const resetEquipmentData = async () => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      const currentUserName = localStorage.getItem('zhihui_site_username') || '匿名用户'
      await dataService.addLog('info', `${currentUserName} 重置了所有设备数据`)
      
      ElMessage.success('设备数据已成功重置')
    } catch (error) {
...
const handleOperatorChange = (selectedName) => {
  if (selectedName) {
    const selectedPerson = constructionPersonnel.value.find(p => p.name === selectedName)
    if (selectedPerson) {
      form.operatorPhone = selectedPerson.phone
    }
  } else {
    form.operatorPhone = ''
  }
