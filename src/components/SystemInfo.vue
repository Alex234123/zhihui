...
const healthStatus = ref({ status: '良好', type: 'success' })
const alerts = ref([])
...
const testNetworkLatency = async () => {
  try {
    ...
    console.error('Error testing network latency:', error)
    networkLatency.value = '测试失败'
  }
}
...
const testNetworkSpeed = () => {
  ...
  const testImageUrl = 'https://api.ipify.org?format=json&t=' + Math.random()
  ...
  fetch(testImageUrl, {
    ...
    const fileSize = 1
    const speed = (fileSize * 8) / (duration / 1000)
    
    if (speed < 100) {
      networkSpeed.value = '慢 (< 100 Kbps)'
    } else if (speed < 1000) {
      networkSpeed.value = `${speed.toFixed(0)} Kbps`
    } else {
      networkSpeed.value = `${(speed / 1000).toFixed(2)} Mbps`
    }
  }).catch(() => {
    networkSpeed.value = '测试失败'
  })
}
...
const shareSystemInfo = () => {
  try {
    ...
    if (navigator.share) {
      navigator.share({ title: '系统信息', text: shareContent }).then(() => {
