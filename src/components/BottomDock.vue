<template>
  <div class="dock-wrapper-container">
    <div
    ...
    >
      <div class="dock-wrapper">
        <div class="dock" ref="dockItemsRef">
          <div class="dock-inner-liquid">
            <div class="dock-liquid-shimmer"></div>
            <div class="dock-liquid-wave"></div>
          </div>
          <div
...
const updateScales = (hoveredIndex) => {
  menuItems.value.forEach((_, index) => {
    ...
    if (distance === 0) {
      itemScales.value[index] = 1.5
      itemTilt.value[index] = -15
      itemElevation.value[index] = 20
    } else if (distance === 1) {
      itemScales.value[index] = 1.3
      itemTilt.value[index] = -10
      itemElevation.value[index] = 15
    } else if (distance === 2) {
      itemScales.value[index] = 1.1
      itemTilt.value[index] = -5
      itemElevation.value[index] = 8
    } else {
...
const handleMouseMove = (e) => {
  ...
  dockItems.forEach((item, index) => {
    ...
    const scale = Math.max(1, Math.min(1.5, 1.5 - distance / 300))
    const tilt = Math.max(-15, Math.min(0, (1 - distance / 200) * -15))
    const elevation = Math.max(0, Math.min(20, 20 - distance / 15))
