    ...
    >
      <el-form :model="form" :rules="rules" ref="formRef" label-width="80px" class="personnel-form">
        ...
        <el-form-item label="年龄" prop="age" class="form-item">
          <el-input-number v-model="form.age" :min="18" :max="65" class="form-input" />
        </el-form-item>
        ...
        <el-form-item v-if="form.responsibility === '施工单位'" label="负责人" prop="supervisor" class="form-item">
          <el-input v-model="form.supervisor" placeholder="请输入负责人或所属负责人" class="form-input" />
        </el-form-item>
        ...
        <el-form-item label="入职日期" prop="entryDate" class="form-item">
          <el-date-picker
    ...
    >
      <div class="personnel-details">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ viewForm.gender }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">年龄:</span>
          <span class="detail-value">{{ viewForm.age }}岁</span>
        </div>
        <div class="detail-row">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ viewForm.entryDate }}</span>
        </div>
...
const exportPersonnel = () => {
  try {
    ...
    // 转换为CSV格式
    const headers = ['编号', '姓名', '性别', '年龄', '岗位', '责任主体', '班组', '联系电话', '入职日期']
    const csvContent = [
      headers.join(','),
      ...data.map(item => [
        item.id,
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        headers.forEach((header, index) => {
          switch (header) {
            ...
            case '性别':
            case 'gender':
              item.gender = values[index] || '男'
              break
            case '年龄':
            case 'age':
              item.age = parseInt(values[index]) || 30
              break
            case '岗位':
            case 'position':
...
.card-header::before {
  ...
  background: linear-gradient(90deg, 
    var(--future-primary) 0%, 
    var(--future-secondary) 50%, 
    var(--future-primary) 100%);
  opacity: 0.8;
}
