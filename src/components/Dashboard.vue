          ...
          <div class="progress-comparison-content">
              ...
              <div v-for="item in blockProgress" :key="item.block" class="comparison-item">
                ...
                <div class="comparison-bar-container">
                  <div class="comparison-bar-wrapper">
                    <div class="comparison-bar planned" :style="{ width: safePercent(getPlannedProgress(item)) + '%' }">
                      <span class="bar-value">{{ safePercent(getPlannedProgress(item)) }}%</span>
                    </div>
                  </div>
                  <div class="comparison-bar-wrapper">
                    <div
                      ...
                      :style="{
                        width: safePercent(item.progress) + '%',
                        background: getActualProgressGradient(item)
                      }"
                    >
                      <span class="bar-value">{{ safePercent(item.progress) }}%</span>
                    </div>
                  </div>
                </div>
            ...
            <div class="comparison-summary">
              <div class="summary-item">
                ...
                <span class="summary-value planned">{{ safePercent(getOverallPlannedProgress()) }}%</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">总体实际进度：</span>
                <span class="summary-value actual">{{ safePercent(getOverallActualProgress()) }}%</span>
              </div>
              <div class="summary-item">
                <span class="summary-label">进度偏差：</span>
                <span class="summary-value" :class="getDeviationClass()">
                  {{ safePercent(getProgressDeviation()) > 0 ? '+' : '' }}{{ safePercent(getProgressDeviation()) }}%
                </span>
              </div>
            </div>
          </div>
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px" class="custom-form">
        ...
        <el-form-item label="负责人" prop="manager">
          <el-select
            ...
            class="custom-input"
            filterable
            @change="handleDashboardManagerChange"
          >
            <el-option
        ...
        <el-form-item label="联系电话" prop="phone">
          <el-input v-model="form.phone" placeholder="选择负责人后自动填充" class="custom-input" disabled />
        </el-form-item>
...
const handleDashboardManagerChange = (selectedName) => {
  if (selectedName) {
    const selectedPerson = constructionPersonnel.value.find(p => p.name === selectedName)
    if (selectedPerson) {
      form.phone = selectedPerson.phone
    }
  } else {
    form.phone = ''
  }
...
const blockCarouselIndex = ref({ A: 0, B: 0, C: 0, D: 0 })
const carouselHover = ref({ A: false, B: false, C: false, D: false })
const carouselTimers = ref({})
...
const prevCounts = ref({ personnel: 0, equipment: 0, safety: 0, materials: 0 })
const todayStr = new Date().toLocaleDateString()
const personnelTrend = computed(() => {
  const todayItems = personnelList.value.filter(p => p.date === todayStr || p.createdAt?.includes(todayStr))
  return todayItems.length
})
const equipmentTrend = computed(() => {
  const todayItems = equipmentList.value.filter(e => e.date === todayStr || e.createdAt?.includes(todayStr))
  return todayItems.length
})
const safetyTrend = computed(() => {
  const todayItems = safetyList.value.filter(s => s.inspectionDate === todayStr || s.date === todayStr || s.createdAt?.includes(todayStr))
  return todayItems.length
})
const materialTrend = computed(() => {
  const todayItems = materialList.value.filter(m => m.inboundDate === todayStr || m.date === todayStr || m.createdAt?.includes(todayStr))
  return todayItems.length
})

function computeBlockStageInfo(blockLetter) {
  if (!progressDetailList.value || progressDetailList.value.length === 0) {
    return { stage: '', currentNode: '' }
  }

  const area = progressDetailList.value.find(a => a.areaId === `area-${blockLetter}`)
  if (!area || !area.stages) return { stage: '', currentNode: '' }

  for (const stage of area.stages) {
    for (const subStage of stage.subStages) {
      const allCompleted = subStage.tasks.every(t => t.status === 'completed' || t.progress >= 100)
      if (allCompleted) continue

      const activeTask = subStage.tasks.find(t => {
        const p = t.progress || 0
        return p > 0 && p < 100
      }) || subStage.tasks.find(t => (t.progress || 0) > 0) || subStage.tasks.find(t => !t.status || t.status === 'not_started' || t.status === 'delayed')

      return {
        stage: subStage.name,
        currentNode: activeTask ? activeTask.name : subStage.name
      }
    }
  }

  return { stage: '已完成', currentNode: '' }
}
...
const blockProgress = computed(() => {
  ...
  return blocks.map(block => {
    ...
    const detailInfo = computeBlockStageInfo(block)

    return found ? {
      ...found,
      stage: detailInfo.stage || found.stage || '',
      currentNode: detailInfo.currentNode || found.currentNode || ''
    } : {
      block,
      manager: '',
      phone: '',
...
const getOverallPlannedProgress = () => {
  ...
  // 只统计有有效计划日期的区块
  const blocksWithPlan = blocks.filter(item => {
    if (!item.targetDate) return false
    if (item.targetDate === '未设定' || item.targetDate === '未设置') return false
    const date = new Date(item.targetDate)
    return !isNaN(date.getTime())
  })
  
  if (blocksWithPlan.length === 0) return 0
  
  const total = blocksWithPlan.reduce((sum, item) => sum + getPlannedProgress(item), 0)
  const result = Math.round(total / blocksWithPlan.length)
...
const getOverallActualProgress = () => {
  const blocks = blockProgress.value
  if (blocks.length === 0) return 0
  const total = blocks.reduce((sum, item) => {
    const progress = Number(item.progress) || 0
    return sum + (isNaN(progress) ? 0 : progress)
  }, 0)
  const result = Math.round(total / blocks.length)
  return isNaN(result) ? 0 : result
}
...
const submitForm = async () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      const existingData = index > -1 ? progressList.value[index] : {}
      const detailInfo = computeBlockStageInfo(form.value.block)
      const updatedData = {
        ...existingData,
        ...form.value,
        stage: detailInfo.stage || form.value.stage || existingData.stage || '',
        currentNode: detailInfo.currentNode || form.value.currentNode || existingData.currentNode || ''
      }
      if (index > -1) {
        progressList.value[index] = updatedData
      } else {
        progressList.value.push(updatedData)
      }
...
const slideCarousel = (block, direction) => {
  const photos = getBlockPhotos(block)
  if (photos.length <= 1) return
  const currentIndex = blockCarouselIndex.value[block] || 0
  let newIndex = currentIndex + direction
  if (newIndex < 0) newIndex = photos.length - 1
  if (newIndex >= photos.length) newIndex = 0
  blockCarouselIndex.value[block] = newIndex
}

const goToSlide = (block, idx) => {
  blockCarouselIndex.value[block] = idx
}

const startAutoCarousel = (block) => {
  stopAutoCarousel(block)
  const photos = getBlockPhotos(block)
  if (photos.length <= 1) return
  carouselTimers.value[block] = setInterval(() => {
    if (!carouselHover.value[block]) {
      slideCarousel(block, 1)
    }
  }, 3500)
}

const stopAutoCarousel = (block) => {
  if (carouselTimers.value[block]) {
    clearInterval(carouselTimers.value[block])
    delete carouselTimers.value[block]
  }
}

const resumeCarousel = (block) => {
  startAutoCarousel(block)
}
...
const subscribeToDataChanges = () => {
  ...
  dataService.subscribe('progressDetail', (data) => {
    if (data) {
      if (Array.isArray(data)) {
        progressDetailList.value = data
        ...
        Object.keys(data).forEach(areaId => {
          ...
          if (areaData && typeof areaData === 'object') {
            let floorData = null
            if (areaData['1']) floorData = areaData['1']
            else if (areaData['2']) floorData = areaData['2']
            else if (areaData['3']) floorData = areaData['3']
            else {
              const keys = Object.keys(areaData)
              if (keys.length > 0) floorData = areaData[keys[0]]
            }
            if (floorData && floorData.stages) {
              converted.push({ areaId, stages: floorData.stages })
            }
          }
        })
        progressDetailList.value = converted
      }
    }
  })
...
onMounted(async () => {
  ...
  setTimeout(() => {
    ['A', 'B', 'C', 'D'].forEach(block => {
      if (getBlockPhotos(block).length > 1) startAutoCarousel(block)
    })
  }, 3000)
})
...
onUnmounted(() => {
  ...
  // 清理轮播定时器
  Object.keys(carouselTimers.value).forEach(block => stopAutoCarousel(block))
})
</script>
...
.stat-card:hover {
  transform:
    ...
    scale(1.02);
  box-shadow:
    0 20px 48px rgba(102, 0, 153, 0.22),
    0 8px 24px rgba(102, 0, 153, 0.15),
...
.legend-color.actual {
  /* background set dynamically via getOverallProgressGradient */
}
...
.carousel-slide .block-photo:hover {
  transform: none;
  box-shadow: none;
}
...
.carousel-dot.active {
  background: #fff;
  box-shadow: 0 0 6px rgba(102, 0, 153, 0.6);
  width: 16px;
  border-radius: 3px;
}
